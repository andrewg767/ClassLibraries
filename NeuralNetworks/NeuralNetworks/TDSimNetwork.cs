...
namespace NeuralNetworks
{
    public class TDSimNetwork : IEvo
    {
        ...
        public List<double[]> TDSim(Network controller)
        {
        ...
        public double Fitness(Network controller)
        {
            ...
            List<double[]> states = TDSim(controller);
            
            for (int sp = 0; sp < SetPoint.Count; sp++)
            {
                for (int i = 0; i < SimTimeSteps; i++)
                {
                    for (int j = 0; j < NumOutputs; j++)
                    {
                        output += Math.Pow(states[i][j] - SetPoint[sp][j],2);

                        if(Math.Abs(states[i][j] - SetPoint[sp][j])> 0.1)
                        {
                            output += Math.Pow(states[i][j] - SetPoint[sp][j], 2);
                        }
                    }
